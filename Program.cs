...
namespace KALISTA
{
    class Program
    {
        ...
        static void Loading_OnLoadingComplete(EventArgs args)
        {
            ...
            Kalistamenu.Add("useq",new CheckBox("Q in COMBO"));
            Kalistamenu.AddSeparator();
			Kalistamenu.Add("combo2",new CheckBox("Let's Fly", false));
            Kalistamenu.AddSeparator();
            Kalistamenu.Add("aad",new CheckBox(""));
        ...
        static void Drawing_OnDraw(EventArgs args)
        {
            if (Kalistamenu["aad"].Cast<CheckBox>().CurrentValue)
            {
                ...
                Drawing.DrawCircle(ObjectManager.Player.Position,range,System.Drawing.Color.Red);
            }
            foreach(var hero in HeroManager.Enemies.Where(x => !x.IsDead && x.GetBuffCount("kalistaexpungemarker")>1))
            {
                if (hero.Health>10)
                {
                    if (hero.GetBuffCount("kalistaexpungemarker")==1)
                    {
                        Drawing.DrawText(Drawing.WorldToScreen(hero.Position)[0],Drawing.WorldToScreen(hero.Position)[1],System.Drawing.Color.Red,(ushort)(100*ObjectManager.Player.CalculateDamageOnUnit(hero, DamageType.Physical,new float[] { 20, 30, 40, 50, 60 }[E.Level-1] + ObjectManager.Player.TotalAttackDamage*60/100)/hero.Health)+" %",3);
                    }
                    if (hero.GetBuffCount("kalistaexpungemarker")>1)
                    {
                        Drawing.DrawText(Drawing.WorldToScreen(hero.Position)[0],Drawing.WorldToScreen(hero.Position)[1],System.Drawing.Color.Red,(ushort)(100*ObjectManager.Player.CalculateDamageOnUnit(hero, DamageType.Physical,new float[] { 20, 30, 40, 50, 60 }[E.Level-1]+ObjectManager.Player.TotalAttackDamage*60/100+(hero.GetBuffCount("kalistaexpungemarker")-1)*(new float[] { 10, 14, 19, 25, 32 }[E.Level-1]+(ObjectManager.Player.TotalAttackDamage*new float[] { 200, 225, 250, 275, 300 }[E.Level-1]/1000)))/hero.Health)+" %",3);
                    }
                }
            }
            foreach (var minion in ObjectManager.Get<Obj_AI_Minion>().Where(x=>!x.IsDead && x.GetBuffCount("kalistaexpungemarker")>3))
            {
                if (minion.Health>10)
                {
                    Drawing.DrawText(Drawing.WorldToScreen(minion.Position)[0],Drawing.WorldToScreen(minion.Position)[1],System.Drawing.Color.Red,(ushort)(100*ObjectManager.Player.CalculateDamageOnUnit(minion, DamageType.Physical,new float[] { 20, 30, 40, 50, 60 }[E.Level-1]+ObjectManager.Player.TotalAttackDamage*60/100+(minion.GetBuffCount("kalistaexpungemarker")-1)*(new float[] { 10, 14, 19, 25, 32 }[E.Level-1]+(ObjectManager.Player.TotalAttackDamage*new float[] { 200, 225, 250, 275, 300 }[E.Level-1]/1000)))/minion.Health)+" %",3);
                }
            }
        }
        static void Items()
